...
export default function QuizApp() {
  ...
  const handleBack = () => {
    if (current > 0) {
      setCurrent(current - 1);
      setSelected(null); // ou conserver la sélection précédente si besoin
    }
  };
  ...
  return (
    <div style={{ maxWidth: 600, margin: "2rem auto", padding: "1rem", border: "1px solid #ccc", borderRadius: 8, fontSize }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1rem" }}>
        ...
        <button onClick={handleRestart} style={{ fontSize, padding: "0.25rem 0.75rem", backgroundColor: "#ddd", border: "none", borderRadius: 4, cursor: "pointer" }}>Recommencer</button>
      </div>
